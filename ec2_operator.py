  ...
  try:
    ...
    for res in reservations:
      for inst in res.instances:
        ...
        if start_sched != None and state == "stopped" and time_to_action(start_sched, now, 15 * 60):
          start_list.append(inst.id)

        # queue up instances that have the stop time falls between 15 minutes ago and now
        if stop_sched != None and state == "running" and time_to_action(stop_sched, now, 15 * -60):
